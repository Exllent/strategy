...
@pytest.mark.parametrize("route_name, form_name", [
...
@pytest.mark.parametrize("route_name, status_code, target_url", [
    ("register", 302, "home"),
    ("register2", 302, "home"),
    ("login", 302, "home"),
    ("logout", 302, "register"),
...
@pytest.mark.django_db
def test_user_redirect(client, create_user, create_castle, route_name, status_code, target_url):
    client.force_login(create_user)
    response = client.get(reverse_lazy(route_name))
    assertRedirects(response, expected_url=reverse_lazy(target_url), status_code=status_code, target_status_code=200)


@pytest.mark.parametrize("route_name, status_code", [
    ("register", 200),
    ("register2", 200),
    ("login", 200),
...
def test_user_login(client, create_castle, create_user):
    response = client.post(reverse_lazy("login"), data={
        "username": 'login_user',
        "password": 'password_user'})

    assertRedirects(response=response, expected_url='/', status_code=302,
                    target_status_code=200)


def test_user_logout(client):
    response = client.get(reverse_lazy("logout"))
    assertRedirects(response, expected_url=reverse_lazy("register"), status_code=302, target_status_code=200)


@pytest.mark.django_db
def test_register(client):
    response = client.post(reverse_lazy("register"), data={"login": "login",
                                                           "email": "email@gmail.com",
                                                           "password1": "asdfgh12345",
                                                           "password2": "asdfgh12345"
                                                           })
    assertRedirects(response, expected_url=reverse_lazy("authorization_code"),
...
def test_authorization_email(client):
    ...
    assertRedirects(response, expected_url=reverse_lazy("register2"),
                    status_code=302, target_status_code=200)
    return client
...
def test_second_register(client, create_race):
    ...
    assertRedirects(response, expected_url=reverse_lazy("home"),
                    status_code=302, target_status_code=200)
