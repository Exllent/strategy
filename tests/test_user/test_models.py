...
@pytest.mark.parametrize(
    "field_name, attribute, result",
    [
        ("login", "max_length", 150),
        ("login", "unique", True),
        ("login", "verbose_name", "Логин"),
        ("nickname", "max_length", 150),
        ...
        ("is_staff", "verbose_name", "Статус персонала"),
        ("is_active", "default", True),
        ("is_active", "verbose_name", "Активность"),
        ...
        ("army", "null", True),
        ("army", "blank", True),
        ("army", "verbose_name", "Армия"),
        ("race", "null", True),
        ("race", "blank", True),
        ("race", "verbose_name", "Раса"),
        ("castle", "null", True),
        ("castle", "blank", True),
        ("castle", "verbose_name", "замок"),
        ("silver_money", "verbose_name", "Серебро"),
        ("silver_money", "default", 0),
        ("gold_money", "verbose_name", "Золото"),
...
def test_validators(django_user_model, field_name, result):
    assert (
        django_user_model._meta.get_field(field_name).validators[-1].limit_value
        == result
    )
