...
def create_resource_buildings(model_resource_buildings, pk):
    ...
    model_resource_buildings.objects.create(name='Каменоломня', resource='stones', castle_id=pk)
    model_resource_buildings.objects.create(name='Шахта', resource='iron', castle_id=pk)
    model_resource_buildings.objects.create(name='Ферма', resource='food', castle_id=pk)
    model_resource_buildings.objects.create(name='Лесопилка', resource='wood', castle_id=pk)
...
class CastleProductionMixin:
    ...
    def get(self, request, **kwargs):
        ...
        resource_buildings = self.model_resource_buildings.objects.filter(castle_id=castle.pk)
        return render(request=request, template_name=self.template_name, context={"resource": resource_buildings})
    ...
    def post(self, request, **kwargs):
        ...
        if request.POST.get('resource'):
            ...
            self.model_resource_buildings.objects.filter(name=building).update(level=F('level') + 1)
