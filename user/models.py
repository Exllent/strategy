...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    login = models.CharField(max_length=150, unique=True, validators=[login_validator],
                             ...
                             verbose_name="Логин")
    nickname = models.CharField(max_length=150, validators=[nickname_validator], blank=True, unique=True,
                                error_messages={"unique": "Пользователь с этим nickname уже существует"},
                                verbose_name='Имя игрока')
    email = models.EmailField(blank=True, error_messages={"unique": "Пользователь с этой почтой уже существует"},
                              unique=True,
                              verbose_name='E-mail')
    is_staff = models.BooleanField(default=False, verbose_name="Статус персонала")
    is_active = models.BooleanField(default=True, verbose_name='Активность')
    is_registered = models.BooleanField(default=False, verbose_name="Статус регистрации")
    date_joined = models.DateTimeField(default=timezone.now, verbose_name='Последний вход')
    slug = models.SlugField(max_length=25, verbose_name='Слаг', unique=True, blank=True)
    army = models.ForeignKey(to=Army, on_delete=models.CASCADE, null=True, blank=True, verbose_name='Армия')
    race = models.ForeignKey(to=Race, on_delete=models.PROTECT, null=True, blank=True, verbose_name='Раса')
    ...
    silver_money = models.PositiveBigIntegerField(default=0, validators=[Max(limit_value=1_000_000)],
                                                  verbose_name='Серебро')
    gold_money = models.PositiveSmallIntegerField(default=0, validators=[Max(limit_value=50_000)],
                                                  verbose_name='Золото')
