...
class UserSecondRegisterForm(forms.Form):
    nickname = forms.CharField(label="Ник-нэйм", max_length=25, min_length=4, help_text='Введите свой ник-нейм')
    castle_name = forms.CharField(label="Название замка", max_length=25, min_length=4,
                                  help_text='Введите название своего замка')
    race = forms.CharField(label='Великая раса', widget=forms.Select(choices=RaceChoices.choices),
...
class UserFirstRegisterForm(UserCreationForm):
    ...
    password1 = forms.CharField(label='Пароль', help_text='password', widget=forms.PasswordInput())
    password2 = forms.CharField(label='Повторите пароль', help_text='Repeat password', widget=forms.PasswordInput())
